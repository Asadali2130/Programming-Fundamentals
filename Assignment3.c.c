...
int checkTilesFit(int plot_width, int plot_length, int tile_width, int tile_length)
{
    ...
    if (plot_width >= tile_width && plot_length >= tile_length)
        return 1; 
    else
        return -1; 
...
int calculateTiles(int plot_width, int plot_length, int tile_width, int tile_length)
{
    ...
    if (tile_width <= 0 || tile_length <= 0) {
        printf("Error: Invalid tile dimensions.\n");
        return -1; 
    }

    int tWidth = plot_width / tile_width;
    int tLength = plot_length / tile_length;
    return tWidth * tLength;
}
